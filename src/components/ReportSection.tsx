...
export default function ReportSection({
    ...
    switch (reportType) {
      case 'monthly_usage': {
        ...
        fulfilledRequests.forEach(r => {
          ...
          r.items.forEach(it => {
            const cat = catalog.find(x => x.id === it.itemId);
            const unitCost = it.unitCostSnapshot || (cat ? cat.unitCost : 0);
            counts[monthYear].quantity += it.quantity;
            counts[monthYear].cost += it.quantity * unitCost;
        ...
        return Object.entries(counts).map(([month, data]) => ({
          ...
          rawCost: data.cost,
          excelRow: [month, String(data.items.size), String(data.quantity), String(data.cost)]
        }));
      }
      ...
      case 'department_consumption': {
        ...
        fulfilledRequests.forEach(r => {
          ...
          r.items.forEach(it => {
            ...
            counts[deptName].cost += it.quantity * unitCost;
          });
        });
        ...
        return Object.entries(counts).map(([dept, data]) => ({
          ...
          col4: formatCurrency(data.cost),
          rawCost: data.cost,
          excelRow: [dept, String(data.ticketCount), String(data.quantity), String(data.cost)]
        }));
      }
      ...
      case 'user_request': {
        ...
        fulfilledRequests.forEach(r => {
          ...
          r.items.forEach(it => {
            ...
            counts[fullName].cost += it.quantity * unitCost;
          });
        });
        ...
        return Object.entries(counts).map(([uName, data]) => ({
          ...
          col4: formatCurrency(data.cost),
          rawCost: data.cost,
          excelRow: [uName, data.email, String(data.requestCount), String(data.cost)]
        }));
      }

      case 'inventory_balance': {
        return catalog.map(item => {
          const totalVal = item.availableQuantity * item.unitCost;
          return {
            col1: item.id,
            col2: item.name,
            col3: `${item.availableQuantity} in stock (Reorder: ${item.reorderLevel})`,
            col4: formatCurrency(totalVal),
            rawCost: totalVal,
            excelRow: [item.id, item.name, String(item.availableQuantity), String(item.reorderLevel), String(totalVal)]
          };
        });
      }

      case 'cost_analysis': {
        const counts: Record<string, { cost: number; units: number }> = {};
        catalog.forEach(cat => {
          counts[cat.category] = { cost: 0, units: 0 };
        });
        ...
        fulfilledRequests.forEach(r => {
          r.items.forEach(it => {
            ...
            const unitCost = it.unitCostSnapshot || (catItem ? catItem.unitCost : 0);

            if (!counts[category]) {
              counts[category] = { cost: 0, units: 0 };
            }
            counts[category].units += it.quantity;
            counts[category].cost += it.quantity * unitCost;
          });
        });

        return Object.entries(counts).map(([cat, data]) => ({
          col1: cat,
          col2: `${data.units} Total units utilized`,
          col3: 'Standard Procurement Line',
          col4: formatCurrency(data.cost),
          rawCost: data.cost,
          excelRow: [cat, String(data.units), String(data.cost)]
        }));
      }

      case 'approval_turnaround': {
        // Simulates realistic turnaround timelines
  ...
  const handleExportCSV = () => {
    ...
    switch (reportType) {
      case 'monthly_usage':
        headers = ['Month/Period', 'Procured Products count', 'Total Quantities', 'Aggregate Cost'];
        filename = 'monthly_stationery_usage.csv';
        break;
      case 'department_consumption':
        headers = ['Department Name', 'Approved ticket Count', 'Sum of Quantities', 'Accumulated Cost'];
        filename = 'dept_stationery_consumption.csv';
        break;
      case 'user_request':
        headers = ['Full Name', 'Corporate Email Address', 'Total order Count', 'Sum cost value'];
        filename = 'user_stationery_reports.csv';
        break;
      case 'inventory_balance':
        headers = ['Catalog ID', 'Product Description', 'Quantity balance', 'Reorder trigger', 'Balance evaluation'];
        filename = 'inventory_stationery_evaluation.csv';
        break;
      case 'cost_analysis':
        headers = ['Product Category', 'Total quantities ordered', 'Aggregate cost expenditure'];
        filename = 'stationery_cost_analysis.csv';
        break;
      case 'approval_turnaround':
    ...
    <div id="reports-section-container" className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1.5 matches-report">
          ...
          >
            ...
            <option value="inventory_balance">Inventory Asset Balance Report</option>
            <option value="cost_analysis">Category Cost Analysis Report</option>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-xs divide-y divide-slate-100 text-xs">
            <div className="pb-3.5 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Total Processed Costs</span>
              <p className="text-xl font-extrabold text-slate-900 font-sans">
                {formatCurrency(
                  fulfilledRequests.reduce((sum, r) => {
                    return sum + r.items.reduce((rowSum, i) => {
                      const c = catalog.find(x => x.id === i.itemId);
                      return rowSum + (i.quantity * (i.unitCostSnapshot || (c ? c.unitCost : 0)));
                    }, 0);
                  }, 0)
                )}
              </p>
            ...
            <div className="py-3.5 space-y-1">
              <span className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Active Catalog valuation</span>
              <p className="text-xl font-bold text-slate-800 font-mono">
                {formatCurrency(catalog.reduce((sum, i) => sum + (i.availableQuantity * i.unitCost), 0))}
              </p>
            </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-[#f8fafc] text-[10px] text-slate-400 font-bold uppercase border-b border-slate-150">
                <tr>
                  <th className="p-3.5 pl-6">
                    ...
                    {reportType === 'cost_analysis' && 'Stationery Category'}
                  ...
                  <th className="p-3.5">
                    ...
                    {reportType === 'cost_analysis' && 'Resource utilization detail'}
                    {reportType === 'approval_turnaround' && 'Internal ID'}
                  </th>
                  <th className="p-3.5">
                    {reportType === 'monthly_usage' && 'Total quantites requested'}
                    ...
                    {reportType === 'cost_analysis' && 'Subtle category type'}
              ...
              <tbody className="divide-y divide-slate-150">
                {reportData.map((row, index) => (
                  <tr key={index} className="hover:bg-slate-25/50">
                    <td className="p-3.5 pl-6 font-bold text-slate-800">{row.col1}</td>
                    <td className="p-3.5 font-semibold text-slate-500">{row.col2}</td>
                    <td className="p-3.5 font-mono font-semibold text-slate-700">{row.col3}</td>
